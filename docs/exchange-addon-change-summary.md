...
当前边界：第一版仍以一分钟 Candle 和传统行情图为基础。用户已确认其视觉和信息密度不完全适合 Minecraft 稀疏成交市场；第二版设计见第 10 节，尚未写入生产代码。
...
## 10. 已确认设计、尚未实现：Minecraft 自适应市场机制

### 10.1 当前机制缺口

现有自成交保护只能阻止同一账户与自己成交，不能阻止两个不同 UUID 串通对敲。当前 `ReferencePriceTracker` 样本主要包含：

- 价格；
- 数量；
- 成交时间。

样本不包含买方、卖方、独立参与者数量、交易对集中度或账户贡献占比。因此两个玩家可以通过反复成交影响最新价、VWAP、参考价和熔断基准。

### 10.2 已确认的设计原则

不是只靠事后检测，而是使用主动式市场结构约束：
...
- 所有市场默认启用“滚动价格移动预算 + 单笔参考价影响上限”；
- 单账户和同一交易对具有窗口贡献上限；
- 短时间、同一组合、同方向的重复成交会被折叠或快速降权；
- 超出预算的合法交易采用“成交但降权”，不直接阻断普通交易；
- 检测只负责提高限制强度、告警和审计，不作为唯一防线；
- 持续高置信度操纵采用渐进式限制：降权 → 告警 → 交易对短时冷却 → 多证据成立时市场级保护。
...
- **最新成交价**：最近一笔真实成交；
- **可信参考价**：经过参与面、集中度、时间分散和价格偏离加权后的稳定基准；
- **市场可信度**：低流动性 / 成长中 / 稳定。

价格笼子、熔断和指导行情将以可信参考价为基准。K 线继续显示真实 OHLCV，并叠加可信参考价线；不会修改、隐藏或伪造成交历史。
...
- 默认使用 `5m / 15m / 1h / 6h / 24h` 等更适合稀疏成交的聚合周期；
- 根据有效 Candle 数自动选择宽度和间距；
- 不为无成交时间伪造 Candle；
- 单 Candle、平价、极端价格和长时间空窗均有专用布局；
- 真实 K 线叠加可信参考价线；
- 重新设计 `1x1`、`2x1`、`2x2` 的信息层级。

以上内容目前属于已确认设计，不应被描述为已发布功能。
...
8. 当前防双账户串通方案尚未实现；在下一版完成前，管理员应重点关注少数玩家反复交易导致的参考价变化。

---

## 14. 结论

本轮已完成 Exchange 在 QuickShop-Hikari 6.2.0.11 上的 GUI 兼容修复和完整交易交互闭环，并交付行情地图墙、行情告示牌、专业图表第一版、GUI 时钟、PDC 交易手册、管理员可靠补发及消息升级兼容。最终版本通过 509 项测试，Maven Reactor 7/7 成功。

下一阶段不应继续机械照搬证券市场，而应围绕 Minecraft 的低流动性和少数玩家结构，实施主动式价格移动预算、单笔影响上限、账户/交易对贡献上限、可信参考价、真实 K 线叠加参考线、自适应行情周期，以及带审计和二次确认的管理员热更新 GUI。
